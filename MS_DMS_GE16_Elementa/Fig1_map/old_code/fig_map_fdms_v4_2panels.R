...
col <- pal(ncol.anp)
# col <- oceColorsSalinity(ncol.anp)
# col <- oceColorsViridis(ncol.anp)
...
if (exportimg) {
  png(filename = '~/Desktop/GreenEdge/MS_DMS_GE16_Elementa/Fig1_map/Fig1_2panels_v4.png',
...
points(x=ge_stn_all$lon, y=ge_stn_all$lat, col='black', cex=0.6, pch=19) # unsampled and sampled
points(x=ge_stn_all$lon, y=ge_stn_all$lat, col=ge_stn_all$col, cex=0.6, pch=19) # unsampled and sampled with ANP when available
points(x=ge_fdms$lon, y=ge_fdms$lat, col='black', cex=0.3, pch=16) # sampled
