...
for (iseason in names(ooedms)[2]) {
  ...
  if (exportimg) {png(filename = paste0(opath,"Fig9_MIZ_EDMS_v2_",iseason,".png"), width = 8, height = 10, units = 'cm', pointsize = 8, bg = 'white', res = plotres, type = 'cairo')}
  ...
  for (ilat in rev(rownames(mizedms$A))) {
    ...
    axis(side = 2, at = ybarBticks, las = 1)
    axis(side = 4, at = yfracBticks * sfactor, labels = yfracBticks, las = 1)
    ...
    if (plet == "c") {
      mtext(side = 2, expression("                          EDMS"[MIZ]*", Gg S yr"^-1), line = 3)
      mtext(side = 4, expression("                          -o-   100 x EDMS"[MIZ]*" / EDMS"[OW]), line = 3)
    }
