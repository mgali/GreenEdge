...
# prof <- select(prof, c(stn,depth,cast,year,month,day,scm,dmspt,diat_cen,diat_pen,dino_athec,dino_thec,chrys,crypt,Phaeo,flag,diat_Laf.mgC_L.mic,pras,prym_clumped_mg_L,detritus_mg_L,melo_mg_L))
prof_full <- prof
prof <- select(prof, c(stn,depth,cast,year,month,day,scm,dmspt,diat_Laf.mgC_L.mic,dino_athec,dino_thec,Phaeo,prym_clumped_mg_L,chrys,crypt,flag,pras,detritus_mg_L,diat_cen,diat_pen,Pico_FC,Bact_FC,melo_mg_L))
...
# Add Melosira using carbon from iFCB: Do not use rdiat, causes 5-fold DMSPp overestimation at stn 703 SCM
rdmsp.melo <- 0.004
prof$dmspt.melo.Cifcb <- prof$melo_mg_L * rdmsp.melo * (1/12) * (1/5) * 1e6
prof$dmspt.melo.Cifcb.pc <- 100*prof$dmspt.melo.Cifcb / (fp * prof$dmspt)
print(summary(prof$dmspt.melo.Cifcb.pc))
...
if (exportimg) {png(filename = paste0(opath,"dmspt_biomass_v4_",cmethod,addcd,Stefels,"_Melo.png"), width = 8, height = 12, units = 'cm', pointsize = 8, bg = 'white', res = 600, type = 'cairo')}
...
if (addcd == "_PhaeoCol_detr"){
  dmsppSUM <- rowSums(dfplot[ , c("Dino_A","Dino_T","Chryso","Crypto","Phaeocystis","Flag_other","Prasino","Diat_all","Melosira","Pico_FC","Phaeo_col","Detritus") ], na.rm = T)
} else {
  dmsppSUM <- rowSums(dfplot[ , c("Dino_A","Dino_T","Chryso","Crypto","Phaeocystis","Flag_other","Prasino","Diat_all","Melosira","Pico_FC") ], na.rm = T)  
}
...
prof <- select(prof_full, c(stn,depth,cast,year,month,day,scm,dmspt,diat_Laf.mgC_L.mic,dino_athec,dino_thec,Phaeo,prym_clumped_mg_L,chrys,crypt,flag,pras,detritus_mg_L,diat_cen,diat_pen,Pico_FC,Bact_FC,melo_mg_L,micral,chlc3,but19_like,tchla))
...
prof <- prof[!is.na(prof$scm),c("stn","depth","cast","scm","dmspt","Phaeo","chlc3_2_tchla","but19_like_2_tchla")]
