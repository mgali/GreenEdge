...
ndhist = 2;
ICEC_HIST = nan(size(casts,1),ndhist+2);
...
for ic = 1:size(casts,1)
    ...
    if sum(iday)
        ...
        if sum(idkm)
            ...
            % Find ice concentration during sampling day, n days backwards
            % and 1 day forward
            ihist = find(dayc(ic) == datei);
            ICEC_HIST(ic,:) = ICEC(ic,(ihist-ndhist):ihist+1);
            
        end
    end
end

ICEC_HIST(isnan(ICEC_HIST)) = 0; % put 0 in first casts, not yet Arctic
icecheader = ({'SIC_minus2d' 'SIC_minus1d' 'SIC_day'})';

%% Assign albedo categories

ndaysback = -ndhist:1;
ALB = nan(size(casts,1),3*length(ndaysback));
% ////////////// edit if snow data available ////////////////
snow = nan(size(casts,1),1);
% ///////////////////////////////////////////////////////////
albheader = ({'type_minus2d' 'type_minus1d' 'type_day' 'type_plus1' 'iALB_minus2d' 'iALB_minus1d' 'iALB_day' 'iALB_plus1' 'tALB_minus2d' 'tALB_minus1d' 'tALB_day' 'tALB_plus1'})';
for j = 1:length(ndaysback)
    % [type,ialbedo,talbedo] = f_get_albedo_v1(doy + ndaysback(j),ICEC_HIST(:,j),snow);
    % [type,ialbedo,talbedo] = f_get_albedo_v2(doy + ndaysback(j),ICEC_HIST(:,j),snow);
    [type,ialbedo,talbedo] = f_get_albedo_v3(doy + ndaysback(j),ICEC_HIST(:,j),snow);
    ALB(:,[0 length(ndaysback) 2*length(ndaysback)]+j) = [type ialbedo talbedo];
end
...
t = casts(:,4); t = [t-2; t-1; t; t+1];
figure(), plot(t,[x y1 y2],'o')
legend('SIC','type/2','A')

%% Write out

OUT = [casts dkm ICEC_HIST ALB];
header_out = [castsheader; 'dkm'; icecheader; albheader];
save(sprintf('CASTS_ICE_albedo_matched_%s.mat',date),'OUT','header_out','ICEC_HIST','icecheader','ALB','albheader')
