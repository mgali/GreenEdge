...
% wlmin = 400; wlmax = 700; % PAR
wlmin = 290; wlmax = 400; % UVR
% wlmin = 320; wlmax = 400; % UVA
% wlmin = 290; wlmax = 320; % UVB
...
% dlmwrite(sprintf('%s/PAR0_sbdart_dailyMetrics.GE%s.%s.txt',outpath,num2str(year),ICorAM),OUT_SBDART,'delimiter','\t','-append')

dlmwrite(sprintf('%s/UVR0_sbdart_dailyMetrics.GE%s.%s.txt',outpath,num2str(year),ICorAM),OUT_SBDART,'delimiter','\t','-append')
