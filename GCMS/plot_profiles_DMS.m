...
zsurf = 5.5;
bubsc = 40; % bubble plot scaling factor
...
for j = 1:length(lon) % loop on transects
    ...
    for k = 1:length(jlons) % loop on transect stations
        ...
        if ~isempty(tmp)
            ...
            ce = cc(end);
            if max(jtrans) < 600
                z = tmp(ps==100&cc==c1,13); x = tmp(ps==100&cc==c1,column); xsd = tmp(ps==100&cc==c1,column+1); scm = tmp(ps==100&cc==c1,30);
                if ce ~= c1
                    zb = tmp(ps==100&cc==ce,13); xb = tmp(ps==100&cc==ce,column); xsdb = tmp(ps==100&cc==ce,column+1);  scmb = tmp(ps==100&cc==ce,30);
                end
            else
                z = tmp(ps==5&cc==c1,13); x = tmp(ps==5&cc==c1,column); xsd = tmp(ps~=100&cc==c1,column+1);  scm = tmp(ps==5&cc==c1,30);
                if ce ~= c1
                    zb = tmp(ps==5&cc==ce,13); xb = tmp(ps==5&cc==ce,column); xsdb = tmp(ps~=100&cc==ce,column+1); scmb = tmp(ps==5&cc==ce,30);
                end
            end
            
            tmp1 = [z x xsd scm];
            tmp1 = sortrows(tmp1,1);
            z = tmp1(:,1); x = tmp1(:,2); xsd = tmp1(:,3); scm = tmp1(:,4);
            % Add second cast if present
            if ce ~= c1
                tmp1_b = [zb xb xsdb scmb];
                tmp1_b = sortrows(tmp1_b,1);
                zb = tmp1_b(:,1); xb = tmp1_b(:,2); xsdb = tmp1_b(:,3);  scmb = tmp1_b(:,4);
            end
            
            % ///// Bubble plot /////
            subplot(1,2,1)
            scatter(jlons(k)*ones(size(z)),-z,bubsc*(x+0.1),jcolors(k,:),'filled'), hold on
            if sum(scm)>0, scatter(jlons(k),-z(scm==1),bubsc*(x(scm==1)+0.5),'k','linewidth',3), end
            if ce ~= c1
                scatter(jlons(k)*ones(size(zb)),-zb,bubsc*(xb+0.05),jcolors(k,:),'filled')
                if sum(scmb)>0, scatter(jlons(k),-zb(scmb==1),bubsc*(xb(scmb==1)+0.5),'k','linewidth',3), end
            end
...
%             else, SURF(stnax==jtrans(k),2) = nanmean([x(z<=zsurf); xb(zb<=zsurf)]);
%             end
    ...
    axis([0 xmax2(j) -zmax(j) 0])
    xlabel('DMS (nM)','fontweight','bold','fontsize',18,'fontname','arial')
    box on
    
    % % Authomatic saving: cuts figures, regardless of format
