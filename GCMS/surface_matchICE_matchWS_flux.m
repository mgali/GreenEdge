...
for j = 1:length(lon) % loop on transects
    ...
    for k = 1:length(jlons) % loop on transect stations
        ...
        % Match casts
        ncast = mode(tmp(:,7));
        castmatch = OUT(:,strcmp(header_out,'cast')) == ncast;
        tmp2 = OUT(castmatch,12:14); % Ice
        tmp3 = CTD{castmatch};
...
% icecrit1 = 0.10; icecrit2 = 0.80;
icecrit1 = 0.1; icecrit2 = 0.85;
...
for ii = 1:3
    ...
    text(ii-0.1,ymax+0.1,sprintf('N=%i',sum(icebin==ii)),'fontsize',12,'rotation',90)
end
